...
*   **Exemplo Prático:** Se a IA decide que na Segunda-feira serão oferecidos 'Teatro', 'Circo' e 'Robótica', as turmas irão rotacionar APENAS entre esses três componentes especialistas ao longo de toda a janela de permanência da Segunda-feira. O componente 'Ócio Vivo' continua sendo usado como balanceador. Isso garante que os educadores de Teatro, Circo e Robótica tenham um dia de trabalho mais completo na escola.
...
1.  **Cálculo do "Orçamento":** Com base nos parâmetros do cenário (Nº de Alunos, Capacidade Máx. da Turma, Nº de Slots na Janela e, crucialmente, o **% de Ócio Vivo**), a ferramenta calcula em tempo real o **número máximo de componentes especialistas distintos** que a grade daquele dia comporta.
...
Esta alavanca transforma uma restrição (o % de Ócio) em uma ferramenta estratégica, garantindo uma operação mais enxuta, eficiente e pedagogicamente rica.

### Regra de Negócio para Alocação e Divisão de Turmas (Modelo Otimizado)

Esta regra define como a demanda total de alunos para um produto é dividida em turmas e como os componentes pedagógicos são alocados na grade horária. O objetivo é otimizar o tempo dos educadores especialistas na escola e garantir que todas as crianças (a "Coorte") rotacionem por todas as experiências, usando o componente nexialista "Ócio Vivo" como balanceador do sistema.
...
2.  **Percentual de Ócio Vivo:** A meta de preenchimento da grade com o componente "Ócio Vivo" é um campo editável. A IA tratará este valor como uma **meta ativa**, ajustando a grade para se aproximar o máximo possível desse percentual, sempre respeitando as regras estruturais de cada cenário de convergência.
3.  **Controle Total do Ócio Vivo:** Se o gestor definir o percentual de "Ócio Vivo" para 100%, a IA irá desconsiderar todas as regras de rotação de especialistas e preencherá 100% da grade horária exclusivamente com o componente "Ócio Vivo", garantindo uma grade totalmente focada em momentos de descompressão.
...
*   **Regra do Ócio Vivo:** Para garantir a descompressão pedagógica, **a IA preencherá a grade de forma que aproximadamente o percentual definido pelo gestor seja de "Ócio Vivo"**, distribuído de forma equilibrada.
...
*   **Regra do Ócio Vivo (Hierarquia):**
    1.  As turmas "excedentes" serão **OBRIGATORIAMENTE** de "Ócio Vivo".
    2.  Se, após essa alocação obrigatória, o percentual de Ócio Vivo na grade ainda for inferior à meta definida pelo gestor, a IA substituirá de forma inteligente algumas alocações de especialistas por Ócio Vivo até atingir a meta.
...
*   **Regra do Ócio Vivo (Hierarquia):**
    1.  O(s) primeiro(s) e/ou último(s) slot(s) da janela serão preenchidos **exclusivamente com "Ócio Vivo" para TODAS as turmas**.
    2.  Se o preenchimento das bordas não for suficiente para atingir a meta percentual de Ócio Vivo definida pelo gestor, a IA substituirá alocações de especialistas nos slots centrais por Ócio Vivo até atingir a meta.
