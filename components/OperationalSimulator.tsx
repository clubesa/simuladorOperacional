...
import { cnaes } from '../data/simplesNacional';
...
const ScenarioCard: React.FC<{ title: string; subtitle: string; children: React.ReactNode; className?: string }> = ({ title, subtitle, children, className }) => (
...
const ResultDisplay: React.FC<{ result: { receita: number; custos: number; impostos: number; resultado: number } }> = ({ result }) => (
...
const OperationalSimulator: React.FC = () => {
    ...
    const [fazerState, setFazerState] = useState({
        ...
        cnaeCode: '85.50-3-02',
        creditGeneratingCosts: 1500, // Parte dos "outros custos"
    });

    const handleFazerChange = (field: keyof typeof fazerState, value: any) => {
    ...
    const handleComprarChange = (field: keyof typeof comprarState, value: any) => {
    ...
    const fazerResult = useMemo(() => {
        ...
        const taxResult = calculateTax({
            simulationYear: 2033, // Simulating full reform scenario for strategic decision
            regime: fazerState.regime,
            receita,
            custo: custos,
            presuncao: 32, // Not used in Lucro Real
            pat: false,
            ...
            <div className="grid md:grid-cols-2 gap-8 mt-8">
                ...
                <ScenarioCard title="Cenário 1: Fazer" subtitle="Operação internalizada pela escola.">
                    ...
                    <FormControl label="Regime Tributário">
                        <Select<TaxRegime> value={fazerState.regime} onChange={v => handleFazerChange('regime', v)} options={[TaxRegime.LUCRO_REAL, TaxRegime.LUCRO_PRESUMIDO]} />
                    </FormControl>
                ...
                <ScenarioCard title="Cenário 2: Comprar" subtitle="Parceria estratégica com a LABirintar.">
                    ...
                    <h4 className="font-semibold text-sm uppercase tracking-wider text-[#8c6d59] border-b border-[#e0cbb2] pb-2 my-4 pt-4">Parâmetros Tributários</h4>
                    <FormControl label="Regime Tributário">
                        <Select<TaxRegime> value={comprarState.regime} onChange={v => handleComprarChange('regime', v)} options={[TaxRegime.LUCRO_PRESUMIDO, TaxRegime.LUCRO_REAL]} />
                    </FormControl>
                     <FormControl label="Atividade (CNAE)">
...
export default OperationalSimulator;